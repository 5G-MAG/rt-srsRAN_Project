                                      ...
                                      task_executor&                             ue_executor)
{
  ...
  if (not udp_session.get_bind_address(bind_addr)) {
    logger_du.error("Could not get bind address for F1-U tunnel. ue={} rb={}", ue_index, rb_id);
    return nullptr;
  }
  ...
  std::unique_ptr<f1u_split_gateway_du_bearer> du_bearer = std::make_unique<f1u_split_gateway_du_bearer>(
      ue_index, rb_id, dl_up_tnl_info, du_rx, ul_up_tnl_info, udp_session, *this, gtpu_pcap, peer_port);
  {
